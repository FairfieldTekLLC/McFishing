...
namespace Fisher
{
    public partial class Form1 : Form
    {
        ...
        // Used to add delay when the bobber is found for the first time.
            ...
            using (Bitmap captureBitmap = CaptureRegion(captureRectangle))
            {
                ...
                for (int j = 0; j < captureRectangle.Height; j++)
                    for (int i = 0; i < captureRectangle.Width; i++)
                    {
                        ...
                        {
                            foundBobber = true;
                            // If this is the first time the bobber was found, add a delay.
                            if (timesFound == 1) {
                                Thread.Sleep(1600);
                            }
                            break;
                ...
                else
                {
                    ...
                    RightClickWindow();
                    // Reset the number of times found to add delay again.
                    timesFound = 0; 
                }
        ...
        /* // Need to finish implementing this later
        /// <summary>
        ///     Saves the current region size for the next the program is opened
        /// </summary>
        /// <param name="value"></param>
        static string fileName = "region_size.dat";
        public void SaveRegionSize()
        {
            int x = Int32.Parse(textBox_x.Text);
            TextWriter tw = new StreamWriter(fileName);

            tw.WriteLine(textBox_x);
            tw.WriteLine(textBox_y);
            tw.Close();
        }
        /// <summary>
        ///     Loads the current region size
        /// </summary>
        /// <param name="value"></param>
        public void LoadRegionSize()
        {
            try
            {
                TextReader tr = new StreamReader(fileName);
                textBox_x.Text = tr.ReadLine();
                textBox_y.Text = tr.ReadLine();
                tr.Close();
            }
            catch (FileNotFoundException)
            {
                LogConsole("Saved no saved region was found... creating " + fileName + ".");
